...
// reference output shared with eigenx-kms's pkg/crypto/storage_key.go and
// the launcher's go-tpm-tools/launcher/internal/storage/key_derivation.go.
// All three implementations MUST produce byte-identical output for the
// same mnemonic — divergence would orphan every PD ever LUKS-formatted
// with one impl when another re-opens it. The vectors below were
// generated by running the reference impl on the listed mnemonics (BIP39
// test vectors) and capturing the hex output.
//
// If this test ever fails, the failure is almost certainly a real
// regression: either this package's derivation drifted (DST string, iter
// count, hash) or one of the other impls did. Fix the offender; do NOT
// update the expected vectors without also updating the corresponding
...
func TestDeriveStorageKey_DSTConstantIsStable(t *testing.T) {
	...
	// derivation contract shared with eigenx-kms and the launcher — any
	// drift here means existing PDs become unreadable. If you're
	// intentionally rotating the DST, do it as a parallel _V2 path, not
	// by editing _V1.
